...
func FindUserByUUIDInBatch(uuids []string) (err error, user []model.AuInterviewUser) {
	err = global.GVA_DB.Preload("AuInterviewTimeTable").Where("uuid in ?", uuids).Find(&user).Error
	return err, user
}
